...
function PrepareCityEditor() {
    xOriginal=$1
    yOriginal=$2
    sourceImage="tiles/14/${xOriginal}/${yOriginal}.jpg";
    ...
    if [[ -f "tiles/19/${xRight}/${yTopCheck}.jpg" ]]; then
        echo "Adding top edge..."
        _prepareHorizontalEdge ${yTopCheck} ${xLeft} ${xRight} North
    fi
    if [[ -f "tiles/19/${xRightCheck}/${yBottom}.jpg" ]]; then
        echo "Adding right edge..."
        _prepareVerticalEdge ${xRightCheck} ${yTop} ${yBottom} SouthEast
    fi
    if [[ -f "tiles/19/${xLeft}/${yBottomCheck}.jpg" ]]; then
        echo "Adding bottom edge..."
        _prepareHorizontalEdge ${yBottomCheck} ${xLeft} ${xRight} SouthWest
    fi
    if [[ -f "tiles/19/${xLeftCheck}/${yTop}.jpg" ]]; then
        echo "Adding left edge..."
        _prepareVerticalEdge ${xLeftCheck} ${yTop} ${yBottom} NorthWest
    fi

	gimp -i -b '(create-city-start-image "start.jpg" "edges.png" "start.xcf")' -b '(gimp-quit 0)'

    rm "edges.png"
    rm "start.jpg"
    mv "start.xcf" "14_${xOriginal}_${yOriginal}.xcf"

    gimp "14_${xOriginal}_${yOriginal}.xcf"
}
...
function _prepareHorizontalEdge() {
    ...
    for x in $(eval echo "{$2..$3}"); do
        appendImages="${appendImages} tiles/19/${x}/$1.jpg"
    done
...
function _prepareVerticalEdge() {
    ...
    for y in $(eval echo "{$2..$3}"); do
        appendImages="${appendImages} tiles/19/$1/${y}.jpg"
    done
    appendImages="${appendImages} map-creation-tools/blank.jpg"
    convert ${appendImages} -append "edge.jpg"
    convert "edges.png" "edge.jpg" -gravity $4 -composite "edges.png"
...
PrepareCityEditor ${xOriginal} ${yOriginal}
